...
# ==============================================================================
# Examples:
#   ./manage_cesm.sh create hybrid FHIST lowres 21 years namelist.txt
#   ./manage_cesm.sh build hybrid FHIST_21years_lowres_user_nl_cam 2
...
#SBATCH --array=1-21%1
#SBATCH --output=${HOST_ARCHIVE_PATH}/slurm-%A_%a.out

echo "[SLURM] \$(date +'%Y-%m-%d %H:%M:%S') - Array Job \${SLURM_ARRAY_JOB_ID} Task \${SLURM_ARRAY_TASK_ID} started."
...
# ==============================================================================
# THE ARRAY CONTINUATION LOGIC
# ==============================================================================
if [ "\${SLURM_ARRAY_TASK_ID}" -gt 1 ]; then
    echo "[SLURM] Task \${SLURM_ARRAY_TASK_ID} > 1 detected. Toggling CONTINUE_RUN=TRUE..."

    # RESCUE THE RESTART FILES: Copy the latest restart files back to the run directory
    echo "[SLURM] Restoring restart files from the archiver..."
    LATEST_REST=\$(ls -td ${HOST_ARCHIVE_PATH}/rest/* | head -1)
    if [ -n "\$LATEST_REST" ]; then
        cp -v \${LATEST_REST}/* ${HOST_SCRATCH_DIR}/${CASE_NAME}/run/
    fi

    # # Spin up a fast, temporary container just to run xmlchange before the main run
    ...
    # Spin up a fast, temporary container to update XML and regenerate Fortran namelists
    srun --nodes=1 --ntasks=1 \\
        podman-hpc run --rm --pull=never $AUTH \\
        ${VOLUMES} \\
        -w /cases/${CASE_NAME} \\
        "${CONTAINER_IMAGE}" \\
        /bin/bash -c "./xmlchange CONTINUE_RUN=TRUE && ./preview_namelists"

else
    echo "[SLURM] Task 1 detected. Starting fresh run (CONTINUE_RUN=FALSE)."
fi
# ==============================================================================
...
srun --mpi=pmi2 ${SRUN_GPU_STR} \\
    ...
    "${CONTAINER_IMAGE}" \\
    /root/cesm/scratch/${CASE_NAME}/bld/cesm.exe > /dev/null
...
if [ \$EXIT_CODE -eq 0 ]; then
    ...
    METADATA_FILE="${HOST_ARCHIVE_PATH}/timing_metrics.csv"
    # If this is the first task (or file doesn't exist), write the CSV header
    if [ ! -f "\$METADATA_FILE" ]; then
        echo "ArrayTaskID,StartDate,EndDate,Duration(s),SimMonths,CalcPerMonth(s),Nodes,CalcPerNode(s),NodeHoursPerMonth" > "\$METADATA_FILE"
    fi
    
    # Append the timing data for this specific task
    echo "\${SLURM_ARRAY_TASK_ID},\$JOB_START_DATE,\$JOB_END_DATE,\$DURATION,\$SIM_MONTHS,\$CALC_PER_MONTH,${NUM_NODES},\$CALC_PER_NODE,\$CALC_PER_MONTH_PER_NODE" >> "\$METADATA_FILE"
