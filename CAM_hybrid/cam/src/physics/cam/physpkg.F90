...
!$OMP PARALLEL DO PRIVATE (C, phys_buffer_chunk)
  ...
  subroutine phys_run2(phys_state, ztodt, phys_tend, pbuf2d,  cam_out, &
    ...
    ! --- FIX START: Import time manager to get current simulation time ---
    ...
    ! --- FIX START: Add all variables needed for get_curr_date ---
    integer :: yr, mon, day, sec  ! year, month, day, seconds
    integer, parameter :: six_hours_in_seconds = 21600
    ...
    ! --- FIX START: Add conditional check for 6-hour interval ---
    ! Use modulo to check if the current time in seconds is an even multiple of 6 hours.
    if (mod(sec, six_hours_in_seconds) == 0) then
        if (masterproc) then
            print*, "================================================="
            print*, "HYBRID MODEL: Calling torch_inference at year: ", yr, ", month: ", mon, ", day: ", day, ", sec: ", sec
            print*, "================================================="
        end if
        call torch_inference(phys_state)
    end if
